...
- Registers three Cavecrew subagents and six Caveman slash commands at runtime, without writing their definitions to the host config. With explicit `options.hostSync: true`, adds only external plugin selectors to the machine's existing global OpenCode config; restart OpenCode to load newly added external plugins.
